"""Jednotkové testy pre moduly feature engineeringu.

Testy overujú základné invarianty troch rodín príznakov:

* Fourierove harmoniky - prítomnosť očakávaných stĺpcov
  a platnosť rozsahu hodnôt (sínus a kosínus musia byť v [-1, 1]).
* České sviatky - korektný výpočet Veľkonočnej nedele
  (Gaussov algoritmus) a správna detekcia Nového roka.
* kategoriálne kódovanie - sanitizácia názvov stĺpcov pre XGBoost
  a LightGBM, ktoré netolerujú špeciálne znaky a medzery.

Testy nepotrebujú skutočné dáta - konštruujú minimalistické
DataFrame priamo v setUp/v tele testovacej metódy.
"""
...
class TestFourierFeatures(unittest.TestCase):
    ...
    def test_fourier_adds_expected_columns(self) -> None:
        ...
        # add_fourier_features needs 'day_of_week' and 'hour' columns
