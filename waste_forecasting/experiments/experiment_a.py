"""Experiment A - predikcia plnosti v momente vývozu.
...
2. Zostavenie datasetu vývozov a pridanie lagov a kĺzavých príznakov.
...
8. Tréning všetkých troch modelov s voliteľným Optuna ladením.
...
def run_experiment_A(
    ...
    """Spustiť Experiment A: predikciu plnosti kontajnera pri vývoze.

    Funkcia spustí Experiment A od detekcie vývozov po export výsledkov.
    Pracuje s kópiou vstupného DataFrame, aby nezmenila pôvodné dáta volajúceho.
    """
    logger.info("=" * 70)
    logger.info("Experiment A: predikcia plnosti pri vývoze naprieč modelmi")
    logger.info("=" * 70)
    ...
    # Odfiltrovanie kontajnerov s malým počtom vývozov.
    ...
    logger.info(f"Tréning: {len(train_containers)} kontajnerov, {len(df_train_all):,} vzoriek")
    logger.info(f"Test: {len(test_containers)} kontajnerov, {len(df_test):,} vzoriek")

    # Uchovanie stĺpca trash_type pred kódovaním pre neskoršiu
    # analýzu predikcií podľa typu odpadu.
    ...
    logger.info(f"Počet príznakov: {len(feature_cols)}")
    ...
    # --- Referenčný model poslednej hodnoty na rovnakom filtri vzoriek
    # Referenčný model predikuje plnosť z predchádzajúceho vývozu (target_lag_1).
    # Vzorky, pre ktoré referenčný model nemá definovanú hodnotu (prvý vývoz
    ...
    logger.info(f"Porovnanie s referenčným modelom: {n_valid}/{n_total} vzoriek ({100*n_valid/n_total:.1f} %)")
    ...
    # --- Tréning a vyhodnotenie viacerých modelov --------------------
    model_results = {}
    primary_model = None
    ...
    for model_name in CONFIG.MODELS_TO_COMPARE:
        if not is_model_available(model_name):
            logger.warning(f"{model_name} nie je dostupný, preskakujem")
            continue

        logger.info(f"\n{'─' * 50}")
        logger.info(f"Trénujem {model_name}...")
        logger.info(f"{'─' * 50}")
        ...
        # Dve sady metrík: na rovnakom filtri ako referenčný model
        # a na celej testovacej množine pre hlavný report.
        ...
        logger.info(f"  {model_name} (rovnaký filter, n={test_metrics['n']}):")
    ...
    # --- Porovnanie modelov voči referenčnému modelu -----------------
    comparison_rows = []
    ...
    logger.info("\nPorovnanie modelov (Experiment A):")
    ...
    logger.info(f"Použité meteorologické príznaky: {weather_feature_count}")
