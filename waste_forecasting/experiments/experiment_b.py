...
def run_experiment_B(
    ...
    """
    logger.info("=" * 70)
    logger.info("EXPERIMENT B: TIME-BASED PREDICTION (24h) (MULTI-MODEL)")
    logger.info("=" * 70)
    ...
    logger.info(f"Time dataset: {len(df_time):,} samples")
    ...
    # trénovacej množine, nie na train/val delení.
    df_train_all = df_train_full.copy()

    # Zachovanie metadát (kontajner, čas, typ odpadu) pred kódovaním -
    # tieto stĺpce sú potrebné pre neskoršiu per-container vizualizáciu.
    ...
    # Baseline predikuje plnosť spred 24 hodín (alebo aktuálnu, ak
    # 24h lag nie je k dispozícii) - tzv. persistence model.
    ...
    # možné robiť per-container a per-trash-type analýzy.
    ...
    logger.info(f"Saved {len(pred_df)} predictions (with metadata) to {exp_dir}/predictions.csv")
