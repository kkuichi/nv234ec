...
Tretí experiment rieši úlohu time-to-threshold - predikuje počet dní, za
ktoré kontajner dosiahne prah FILL_THRESHOLD (predvolene 85 %). Slúži pre
dlhodobé plánovanie zvozu, ak model povie že kontajner bude plný o 3 dni,
dispatcher má dosť času nechať ho zaradiť do plánu.
...
korektnú alternatívu založenú na right-censoring pozri survival_baseline.py.

Víťaz sa vyberá podľa MAE v dňoch (nie RMSE), pretože MAE je robustnejšia
pre exponenciálne rozdelené cieľové hodnoty, aké bývajú v time-to-event
úlohách bežné.
"""
...
def run_experiment_C(
    ...
    """
    logger.info("=" * 70)
    logger.info(f"EXPERIMENT C: TIME-TO-THRESHOLD ({CONFIG.FILL_THRESHOLD}%) (MULTI-MODEL)")
    logger.info("=" * 70)
    ...
    if len(df_ttt) < 100:
        logger.warning("Insufficient data for Experiment C")
        return None
    ...
    # --- Lineárny baseline ------------------------------------------
    # Baseline používa lineárnu extrapoláciu tempa plnenia
    # (estimated_time_linear z TTT featuru). Ide o prirodzenú
    ...
    for model_name in CONFIG.MODELS_TO_COMPARE:
        ...
        logger.info(f"  {model_name} (days): RMSE={test_metrics['rmse']:.2f}, MAE={test_metrics['mae']:.2f}")
