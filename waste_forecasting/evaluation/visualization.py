...
Modul obsahuje funkcie, ktoré zo CSV výstupov pipeline vytvárajú finálne
obrázky do textu diplomovky aj príloh. Každá väčšia skupina grafov má
vlastnú funkciu, všetky texty sú v slovenčine s diakritikou a generovanie
je robustné voči chýbajúcim vstupom - ak niektorý CSV neexistuje, zlyhá iba
konkrétny obrázok, nie celý beh.
...
def compute_decision_metrics(output_dir: str = "output") -> Dict:
    ...
    if pred_b_path.exists():
        ...
        # Top-k hit rate pre hlavný prevádzkový prah 85 %.
        threshold_main = 85
...
def generate_per_container_figures(
    ...
    if not pred_path.exists():
        logger.info("Per-container vizualizácie pre Experiment B nie sú dostupné.")
        return None

    df = pd.read_csv(pred_path)
    required_cols = {"container_id", "measured_at_utc", "actual", "predicted"}
    if not required_cols.issubset(df.columns):
        logger.info("predictions.csv neobsahuje všetky stĺpce pre per-container vizualizácie.")
        return None

    df["measured_at_utc"] = pd.to_datetime(df["measured_at_utc"])

    # Per-container štatistiky slúžia na výber reprezentatívnych sérií.
    stats_rows = []
...
def generate_thesis_figures(df_raw: pd.DataFrame, output_dir: str = "output") -> None:
    ...
    # --- Obr. 17: súhrnný dashboard ---------------------------------
    try:
        fig, axes = plt.subplots(1, 3, figsize=(14, 5))
        configs = [
            ("exp_A", "model_comparison.csv", "Experiment A\n(event-based)", "rmse", "RMSE (p. b.)"),
            ("exp_B", "metrics.csv", "Experiment B\n(24h time-based)", "rmse", "RMSE (p. b.)"),
            ("exp_C", "metrics.csv", "Experiment C\n(time-to-threshold)", None, "MAE (dni)"),
        ]
