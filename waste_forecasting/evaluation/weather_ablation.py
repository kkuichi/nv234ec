...
Cieľom je odhadnúť, o koľko sa zlepší testovacie RMSE, keď do datasetu
pridám meteorologické príznaky. Postup je priamočiary - trénujem dva
varianty pipeline, jeden s kompletnou sadou meteorologických príznakov
a druhý bez nich, a porovnám testovacie RMSE. Rozdiel dáva odhadovaný
prínos počasia.

Sú k dispozícii dve samostatné funkcie: run_weather_ablation_study pre
event-based úlohu (Experiment A) a run_weather_ablation_expB pre
time-based úlohu (Experiment B). Obe ukladajú výsledky ako CSV tabuľky
v príslušnom podpriečinku výstupu.
"""
...
def run_weather_ablation_study(
    ...
    Štúdia validuje pridanú hodnotu externých meteorologických dát.
    Natrénujú sa dva varianty event-based pipeline (Experiment A):
    s kompletnou sadou meteorologických príznakov a bez nich.
    Porovnanie testovacích RMSE kvantifikuje ich prínos.
    """
    logger.info("=" * 70)
    logger.info("WEATHER ABLATION STUDY: WITH vs WITHOUT WEATHER")
    logger.info("=" * 70)
    ...
    # Filter: vyžadujeme dostatok vzoriek na kontajner pre tréning aj test
    ...
    for variant_name, add_weather in [('WITHOUT_WEATHER', False), ('WITH_WEATHER', True)]:
        ...
        logger.info(f"  {variant_name}: RMSE={metrics['rmse']:.2f}, R2={metrics['r2']:.3f}, Features={len(feature_cols)}")
    ...
    logger.info(f"\nWeather Impact:")
    logger.info(f"  RMSE reduction: {improvement:.2f} ({improvement_pct:+.1f}%)")
    logger.info(f"  Additional features: {results['improvement']['feature_increase']}")
...
def run_weather_ablation_expB(df: pd.DataFrame, test_containers: set, output_dir: str) -> Dict:
    """Ablačná štúdia meteorologických príznakov pre Experiment B (24h).

    Analogický variant k run_weather_ablation_study, ktorý
    namiesto event-based datasetu používa time-based dataset s 24-hodinovým
    predikčným horizontom (Experiment B). Natrénujú sa dva modely -
    s meteorologickými príznakmi a bez nich - pričom variant bez počasia
    navyše filtruje aj akékoľvek odvodené príznaky obsahujúce meteorologické
    kľúčové slová (temp, precip, humid a ďalšie), aby sa
    zamedzilo neúmyselnému úniku počasia cez interakčné členy.

    Na rozdiel od Experimentu A sa tu aplikuje aj vnútorný temporálny
    validačný split (temporal_train_val_split_per_container),
    takže tréningová množina nevidí „budúcnosť" rovnakého kontajnera.
    """
    logger.info("=" * 70)
    logger.info("WEATHER ABLATION: EXPERIMENT B")
    logger.info("=" * 70)
    ...
    for variant, use_weather in [('with_weather', True), ('without_weather', False)]:
        ...
        logger.info(f"  {variant}: {len(feature_cols)} features, RMSE={metrics['rmse']:.2f}, R2={metrics['r2']:.3f}")
    
    with open(f'{abl_dir}/ablation_expB.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)
    
    logger.info(f"  Exp B weather impact: RMSE {results['without_weather']['rmse']:.2f} -> {results['with_weather']['rmse']:.2f}")
