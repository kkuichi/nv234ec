"""Analýza stability výsledkov pri viacerých seedoch.
...
Stabilita sa hodnotí na úlohe predikcie pri vývoze rovnako ako v Experimente A.
...
def run_stability_analysis(df: pd.DataFrame, output_dir: str) -> pd.DataFrame:
    """Vykonať analýzu stability pri viacerých seedoch naprieč všetkými modelmi.
    ...
    naprieč seedmi) ukazujú variabilitu výsledkov pipeline.
    """
    logger.info("=" * 70)
    logger.info("Analýza stability naprieč modelmi")
    logger.info("=" * 70)

    stab_dir = f'{output_dir}/stability'
    os.makedirs(stab_dir, exist_ok=True)

    # --- Jednorazová príprava datasetu vývozov ----------------------
