...
predikuje spojitý počet dní, kým kontajner dosiahne FILL_THRESHOLD.
Tento prístup ale nevie rozumne zaobchádzať s cenzorovaním pravej
strany: kontajnery, ktoré počas sledovaného horizontu prah nedosiahli,
musia byť buď vylúčené (čo vytvára selekčný bias), alebo im priraďujem
hodnotu MAX_TTT_DAYS ako keby to bol skutočný jav (čo skresľuje tréning).

Tu sú preto dva klasické nástroje z analýzy prežívania, ktoré pracujú
s cenzorovaním natívne: Kaplan-Meier (neparametrický odhad funkcie
prežitia, predikcia je konštantný mediánový čas) a Cox Proportional
Hazards (semi-parametrický regresný model s aktuálnou plnosťou ako
jediným prediktorom).

Oba modely volám cez knižnicu lifelines. Ak nie je nainštalovaná,
funkcia vracia slovník s kľúčom error a hlavná pipeline pokračuje
bez survival baseline.
"""
...
def run_survival_baseline_expC(df: pd.DataFrame, test_containers: set, output_dir: str) -> Dict:
    """Spustiť Kaplan-Meier a Cox PH baseline pre Experiment C.

    Pre každé meranie pod prahom CONFIG.FILL_THRESHOLD (typicky 85 %)
    skonštruujem pár (duration, event): čas do najbližšieho dosiahnutia
    prahu, alebo ak prah nebol počas nasledujúcich CONFIG.MAX_TTT_DAYS
    dní dosiahnutý, cenzorovanú hodnotu MAX_TTT_DAYS s event=False.

    Kaplan-Meier trénujem iba na dĺžkach a príznaku udalosti, bez
    kovariátov, a ako predikciu dáva svoj mediánový čas prežitia.
    Cox PH pridáva jednu kovariátu (aktuálnu plnosť) a predikuje
    mediánový čas do prahu podmienený touto hodnotou.

    Vyhodnocujem len na testovacích kontajneroch a iba na vzorkách
    s pozorovanou udalosťou (event_observed=True), aby bolo porovnanie
    s regresným Experimentom C spravodlivé. Pri Coxovi z predikcií
    odstraňujem nekonečné hodnoty (vznikajú keď model odhaduje prah
    mimo sledovaného horizontu) a zvyšné orezávam na [0, MAX_TTT_DAYS].
    """
    logger.info("=" * 70)
    logger.info("SURVIVAL BASELINE: EXPERIMENT C")
    logger.info("=" * 70)
    ...
    if len(surv_df) == 0:
        logger.warning("No survival data generated")
        return {'error': 'no data'}
    
    train_surv = surv_df[~surv_df['is_test']].copy()
    test_surv = surv_df[surv_df['is_test']].copy()
    
    logger.info(f"  Survival data: train={len(train_surv)}, test={len(test_surv)}")
    logger.info(f"  Events observed: train={train_surv['event'].sum()}/{len(train_surv)}, test={test_surv['event'].sum()}/{len(test_surv)}")
    ...
    try:
        ...
        if len(test_events) > 0:
            ...
            logger.info(f"  Kaplan-Meier: median={km_median:.2f}d, RMSE={km_rmse:.2f}d, MAE={km_mae:.2f}d, R2={km_r2:.3f}")
    except Exception as e:
        logger.warning(f"  Kaplan-Meier failed: {e}")
    ...
    try:
        ...
        if len(test_events) > 0:
            ...
            if valid.sum() > 0:
                ...
                logger.info(f"  Cox PH: RMSE={cox_rmse:.2f}d, MAE={cox_mae_val:.2f}d, R2={cox_r2:.3f}, C-index={cph.concordance_index_:.3f}")
    except Exception as e:
