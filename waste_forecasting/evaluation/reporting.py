"""Generovanie finálnych reportov a CSV tabuliek.

Modul je posledný krok pipeline. Zoberie výsledky z jednotlivých
experimentov a uloží ich do FINAL_REPORT.txt a do CSV tabuliek,
ktoré slúžia ako kontrolné výstupy a podklady pre tabuľky v diplomovej práci.

Hlavné funkcie:
- generate_final_report - textový súhrn do FINAL_REPORT.txt
- export_results_table - súhrnná tabuľka výsledkov experimentov A, B, C a baseline modelov
- save_capacity_comparison_table - výsledky po kapacitných triedach
- export_hyperparameter_tables - najlepšie parametre z Optuny
- export_fixed_table16 - multi-seed stabilita per model

Všetky funkcie predpokladajú, že experimenty už prebehli a ich
výstupy sú v output_dir/exp_A, exp_B, exp_C a stability.
"""
...
def generate_final_report(all_results: Dict, output_dir: str):
    ...
    if 'exp_A' in all_results and all_results['exp_A'] is not None:
        ...
        report += f"""
EXPERIMENT A: Event-based Fill Level Prediction
-----------------------------------------------
  Best model: {pname}
  ...
  Baseline (Last-Value): RMSE={b['rmse']:.2f}, R2={b['r2']:.3f}
"""
    ...
    if 'exp_B' in all_results and all_results['exp_B'] is not None:
        ...
        report += f"""
EXPERIMENT B: Time-based Fill Level Prediction (24h)
----------------------------------------------------
  Best model: {pname}
  Test RMSE: {m['rmse']:.2f}
...
"""
    ...
    if 'exp_C' in all_results and all_results['exp_C'] is not None:
        ...
        report += f"""
EXPERIMENT C: Time-to-Threshold Prediction
------------------------------------------
  Best model: {pname}
  Test RMSE: {m['rmse']:.2f} days
  Test MAE:  {m['mae']:.2f} days
  Test R2:   {m['r2']:.3f}
  
  SELECTION BIAS:
    Coverage: {sb.get('n_containers_with_ttt', 0)}/{sb.get('n_containers_total', 0)} ({sb.get('coverage_pct', 0):.1f}%)
"""
...
----------------------
  WITHOUT Weather: RMSE={wa['WITHOUT_WEATHER']['rmse']:.2f}, R2={wa['WITHOUT_WEATHER']['r2']:.3f}
  WITH Weather:    RMSE={wa['WITH_WEATHER']['rmse']:.2f}, R2={wa['WITH_WEATHER']['r2']:.3f}
  
  Impact:
    RMSE reduction: {imp.get('rmse_reduction', 0):.2f} ({imp.get('rmse_reduction_pct', 0):+.1f}%)
    Additional features: {imp.get('feature_increase', 0)}
"""
    
    if 'cv' in all_results and all_results['cv'] is not None:
        cv = all_results['cv']
        report += f"""
CROSS-VALIDATION ({CONFIG.N_FOLDS}-fold, {CONFIG.CV_TYPE})
----------------------------------------
...
"""
    ...
    logger.info(f"Report saved to {output_dir}/FINAL_REPORT.txt")
    print(report)
...
def export_results_table(all_results: Dict, output_dir: str) -> pd.DataFrame:
    ...
    experiment_meta = {
        "exp_A": {
            ...
            "task": "Event-based fill-level prediction",
            "target": "Fill level before inferred collection",
            "metric_unit": "percentage points",
            "primary_metric": "RMSE",
            "baseline_name": "Last Value baseline",
            "order": 1,
        },
        "exp_B": {
            "experiment": "Experiment B",
            "task": "24h fill-level prediction",
            "target": "Fill level 24 hours ahead",
            "metric_unit": "percentage points",
            "primary_metric": "RMSE",
            "baseline_name": "Seasonal baseline",
            "order": 2,
        },
        "exp_C": {
            "experiment": "Experiment C",
            "task": "Time-to-threshold prediction",
            "target": "Days until 85 percent fill level",
            "metric_unit": "days",
            "primary_metric": "MAE",
            "baseline_name": "Linear extrapolation baseline",
            "order": 3,
        },
    }
    ...
    for exp_key in ["exp_A", "exp_B", "exp_C"]:
        ...
        if test_metrics:
            _add_row(
                ...
                notes="Best model selected inside the experiment",
            )
        ...
        if baseline_metrics:
            _add_row(
                ...
                notes="Baseline computed inside the experiment",
            )
    ...
    if baselines is not None and isinstance(baselines, pd.DataFrame) and not baselines.empty:
        ...
        for _, baseline_row in baselines.iterrows():
            ...
            _add_row(
                ...
                metrics=metrics,
                notes="Additional baseline from compare_all_baselines",
            )

            existing_models.add(model_name)

    if not rows:
        logger.warning("No results available for results_table.csv")
        return pd.DataFrame()
    ...
    logger.info("Results table saved to %s", output_path)
...
def export_hyperparameter_tables(output_dir: str = 'output'):
    ...
    Načíta optuna_best_params.json (ten sa generuje v module
    tuning.py) a rozdelí ho do dvoch formátov:
    ...
    if not os.path.exists(params_path):
        logger.error(f"Súbor {params_path} neexistuje.")
        return None
...
def export_fixed_table16(output_dir: str = 'output'):
    ...
    print("\n" + "=" * 70)
    print("TABUĽKA 16 (per-model multi-seed)")
    print("=" * 70)
