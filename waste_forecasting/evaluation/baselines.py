...
Modul implementuje sadu baseline prístupov, ktoré slúžia ako referenčná
úroveň pre hodnotenie hlavných modelov gradient boostingu. Všetky
baseline metódy sú vyhodnocované na identickom filtri vzoriek ako hlavné
modely, takže ich RMSE je priamo porovnateľné.
...
def run_auto_arima_baseline(df: pd.DataFrame, target_col: str = 'target') -> Tuple[Optional[np.ndarray], Dict]:
    ...
    """
    if not HAS_PMDARIMA:
        logger.warning("pmdarima not installed, falling back to fixed ARIMA")
        return run_arima_baseline(df, target_col)
    
    logger.info("Running Auto-ARIMA baseline...")
...
def run_arima_baseline(df: pd.DataFrame, target_col: str = 'target', order: Tuple = (2, 0, 1)) -> Tuple[Optional[np.ndarray], Dict]:
    ...
    """
    if not HAS_ARIMA:
        logger.warning("ARIMA not installed, skipping")
        return None, {'error': 'ARIMA not installed'}
    
    logger.info(f"Running ARIMA{order} baseline...")
...
def run_seasonal_naive_baseline(df: pd.DataFrame, target_col: str = 'target', period: int = 7) -> Tuple[Optional[np.ndarray], Dict]:
    ...
    """
    logger.info(f"Running Seasonal Naive (period={period})...")
...
def run_arima_prophet_baseline_expB(df: pd.DataFrame, test_containers: set, output_dir: str,
    ...
    """
    logger.info("=" * 70)
    logger.info(f"ARIMA/PROPHET BASELINE: EXPERIMENT B (subsample {max_containers} containers)")
    logger.info("=" * 70)
    ...
    horizon_steps = 24 // CONFIG.RESAMPLE_HOURS  # 4 steps for 6h resample
    ...
    for cid in test_ids:
        ...
        if HAS_PMDARIMA:
            try:
                ...
                logger.debug(f"ARIMA failed for container {cid}: {e}")
        ...
        if HAS_PROPHET:
            try:
                ...
                logger.debug(f"Prophet failed for container {cid}: {e}")
    ...
    if arima_preds:
        ...
        logger.info(f"  Auto-ARIMA (subsample): RMSE={arima_rmse_val:.2f}, R2={arima_r2:.3f}, n={len(arima_preds)}")
    ...
    if prophet_preds:
        ...
        logger.info(f"  Prophet (subsample): RMSE={prophet_rmse_val:.2f}, R2={prophet_r2:.3f}, n={len(prophet_preds)}")
