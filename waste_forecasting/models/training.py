"""Wrappery pre tréning gradient boosting modelov a jednotný dispatcher.

Pre každý z troch porovnávaných modelov (HistGradientBoostingRegressor
zo scikit-learn, LightGBM a XGBoost) je v tomto module jeden wrapper
s rovnakou signatúrou: dostane trenovacie dáta (a voliteľne validačnú
množinu) a vráti trojicu (model, y_pred, info). Funkcia train_model
potom podľa textového identifikátora modelu zavolá správny wrapper.
...
def train_histgb_model(
    ...
    if hasattr(model, 'n_iter_'):
        ...
        if model.n_iter_ < max_iter:
            logger.info(f"Early stopping at {model.n_iter_}/{max_iter}")
...
def train_lgbm_model(
    ...
    logger.info(f"LightGBM training time: {train_time:.1f}s, iterations: {info['n_iter']}")
...
def train_xgb_model(
    ...
    logger.info(f"XGBoost training time: {train_time:.1f}s, iterations: {info['n_iter']}")
...
def train_model(
    ...
    """Dispatcher, ktorý podľa názvu modelu volá príslušný wrapper.

    Slúži ako jednotný vstupný bod pre tréning: experimentálny
    kód nepotrebuje vedieť, ktorá konkrétna knižnica je použitá.
    """
