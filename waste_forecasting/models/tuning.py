"""Ladenie hyperparametrov pomocou frameworku Optuna.

Modul implementuje triedu MultiModelTuner, ktorá podľa názvu
modelu zostaví adekvátny parametrický priestor a vykoná temporálnu
krížovú validáciu nad tréningovou množinou. Cieľovou funkciou je
priemerný RMSE naprieč CONFIG.OPTUNA_CV_FOLDS foldmi.

Sampler je deterministický TPESampler(seed=CONFIG.SEED), čo
zaručuje reprodukovateľnosť hľadania. Pre spätnú kompatibilitu
je ponechaná aj pôvodná trieda TemporalHyperparameterTuner,
ktorá pracuje iba s HistGradientBoostingRegressor.
"""
...
class TemporalHyperparameterTuner:
    ...
    def tune(self, df: pd.DataFrame, feature_cols: List[str]) -> Dict:
        ...
        logger.info(f"Starting Optuna ({self.n_trials} trials, {self.cv_folds}-fold temporal CV)")
        ...
        logger.info(f"Best temporal CV RMSE: {self.study.best_value:.4f}")
...
class MultiModelTuner:
    ...
    * XGBoost: analogický priestor ako LightGBM s parametrom
    ...
    def objective(self, trial, splits) -> float:
        ...
        for X_train, y_train, X_val, y_val in splits:
            ...
            if y_pred is not None:
                ...
                # Fallback: ak wrapper nevrátil predikciu, dopočítame ju ručne.
    ...
    def tune(self, df: pd.DataFrame, feature_cols: List[str]) -> Dict:
        ...
        logger.info(f"Starting Optuna for {self.model_name} ({self.n_trials} trials, {self.cv_folds}-fold)")
        ...
        logger.info(f"Best {self.model_name} CV RMSE: {self.study.best_value:.4f}")
