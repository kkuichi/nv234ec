"""Temporálny feature engineering: lag, rolling a experimentálne datasety.

Modul implementuje konštrukciu sád príznakov pre všetky tri experimenty:

* Experiment A (event-based): lag a kĺzavé príznaky počítané nad
  detegovanými vývozmi. Cieľová premenná je plnosť pred vývozom.
* Experiment B (24h time-based): lag a kĺzavé príznaky nad
  pravidelne prevzorkovanou časovou sériou. Cieľová premenná je
  hodnota plnosti o 24 hodín neskôr.
* Experiment C (time-to-threshold): príznaky pre odhad počtu
  dní do dosiahnutia 85 % plnosti, vrátane lineárnej extrapolácie.
"""
...
def create_event_dataset(df: pd.DataFrame) -> pd.DataFrame:
    """Zostaviť event-based dataset pre Experiment A.

    Z kompletnej tabuľky meraní vyberie iba riadky s detegovaným
    vývozom (is_collection == 1) a ako cieľovú premennú priradí
    ...
    logger.info(f"Event dataset: {len(collections):,} events, {collections['container_id'].nunique()} containers")

    return collections


def add_event_features(df: pd.DataFrame) -> pd.DataFrame:
    """Pridať lag a kĺzavé príznaky pre event-based predikciu.
...
def create_time_dataset(df: pd.DataFrame, horizon_hours: int = 24) -> pd.DataFrame:
    """Zostaviť time-based dataset pre Experiment B.
...
def add_time_features(df: pd.DataFrame) -> pd.DataFrame:
    """Pridať lag a kĺzavé príznaky pre time-based predikciu.

    Na rozdiel od event-based verzie pracuje s pravidelnou
...
def create_time_to_threshold_dataset(df_resampled: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
    """Zostaviť dataset pre Experiment C (time-to-threshold).
    ...
    Dôležité upozornenie: funkcia loguje WARNING o selekčnom skreslení
    (SELECTION BIAS), pretože do výsledného datasetu vstupujú iba
    kontajnery, pri ktorých bol aspoň raz pozorovaný prechod cez prah.
    """
    logger.info(f"Creating TTT dataset (threshold: {CONFIG.FILL_THRESHOLD}%)")
    ...
    def calc_ttt(group):
        ...
        for i in range(n):
            if pct[i] >= CONFIG.FILL_THRESHOLD:
                # Už sme nad prahom - time-to-threshold je 0.
                result[i] = 0
    ...
    logger.info(f"TTT dataset: {len(df):,} samples, mean TTT: {df['target'].mean():.1f} days")
    logger.warning(
        f"SELECTION BIAS: TTT dataset covers {n_containers_with_ttt}/{n_containers_total} "
        f"({selection_bias_info['coverage_pct']:.1f}%) containers."
    )
...
def add_ttt_features(df: pd.DataFrame) -> pd.DataFrame:
    """Pridať príznaky pre Experiment C (time-to-threshold).
