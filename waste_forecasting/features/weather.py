"""Meteorologický feature engineering.

Modul zlučuje externé meteorologické dáta (z lokálneho CSV, typicky export
zo stanice Praha-Ruzyně vo formáte Meteostat) so základným datasetom meraní
plnosti a odvodí z nich širokú sadu príznakov.

Do datasetu pribúdajú surové hodnoty (teplota, zrážky, vietor, tlak,
voliteľne sneh a slnečné žiarenie), lag príznaky posunuté o 1, 6, 12 a 24
krokov, rolling štatistiky (priemer, smerodajná odchýlka, min, max) cez
okná 6-48 krokov, kategorické indikátory typu temp_cold, rain_heavy,
wind_strong, indexy pocitovej teploty (Heat Index, Wind Chill) a niekoľko
interakcií počasie-správanie (napríklad teplý víkend, interakcia teploty
a dňa v týždni).

Prínos celej tejto sady je potom kvantifikovaný v ablačnej štúdii
(weather_ablation.py), kde sa porovnáva RMSE s počasím a bez neho.
"""
...
# Modul-level cache: načítanie meteorologického CSV prebehne najviac
    ...
    * snow, tsun - voliteľné sneh a slnečné žiarenie.
    ...
    for c in ["time", "date", "datetime", "Date", "DATE"]:
        if c in w.columns:
            ...
            break
    if dt_col is None:
        logger.warning("Weather CSV nemá datetime stĺpec. Očakáva sa jeden z týchto: time, date, datetime.")
        return None
    ...
    if "tsun" in w.columns:
        w["sunshine_min"] = w["tsun"]  # Meteostat tsun is minutes
    ...
    logger.info(f"Loaded external weather: {resolved}. Rows={len(w):,}")
    return w
...
def merge_external_weather(df: pd.DataFrame, weather_path: Optional[str] = None) -> pd.DataFrame:
    ...
    najbližšia predchádzajúca meteorologická pečiatka (zabraňuje
    look-ahead biasu).
    ...
    # Report pokrytia po merge
    cov = {}
    for c in ["temperature", "precipitation", "wind_speed", "pressure"]:
        if c in df.columns:
            cov[c] = float(df[c].notna().mean())
    logger.info(f"Weather merge coverage: { {k: f'{v*100:.1f}%' for k, v in cov.items()} }")
...
def ensure_weather_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    logger.info("Ensuring weather columns (no synthesis, no bfill, add missing indicators)...")
    df = df.copy()
    ...
    # Triedenie pred forward fill kvôli bezpečnému doplneniu
    ...
    # Ostatné spojité premenné - iba forward fill, nikdy backward
...
def add_temperature_features(df: pd.DataFrame, mode: str = "auto", step_hours: int = None) -> pd.DataFrame:
    ...
    if mode == "time":
        ...
        # Event režim - hodinové okná nedávajú zmysel, používame event-okná
        event_lags = [1, 2, 3]
...
def add_all_weather_features(df: pd.DataFrame, mode: str = "auto", step_hours: int = None) -> pd.DataFrame:
    ...
    Funkcia predstavuje jednotný vstupný bod pre meteorologický
    feature engineering. Interne volá sekvenciu krokov:
    ...
    logger.info(f"Adding weather features. mode={mode}, step_hours={step_hours}")
...
def analyze_weather_impact(df: pd.DataFrame, target_col: str = 'target', output_dir: str = None) -> Dict[str, Any]:
    ...
    if len(corr_df) > 0:
        ...
        logger.info("Top 5 weather correlations:")
