...
Modul implementuje heuristickú detekciu vývozov odpadu, ktorá tvorí proxy
...
def detect_collections(
    ...
    * voliteľne aj kontrola budúceho merania (vypnutá predvolene,
      pretože používa informáciu z budúcnosti).
    """
    ...
    logger.info(f"Detegovaných vývozov: {n_collections:,} (priemer na kontajner: {avg_per_container:.1f})")
...
def sensitivity_analysis_collections(df: pd.DataFrame, output_dir: str) -> pd.DataFrame:
    ...
    kombináciu zaznamená počet detegovaných vývozov. Výsledok ukazuje,
    ako citlivá je detekčná heuristika na voľbu prahov.
    """
    logger.info("=" * 70)
    logger.info("Analýza citlivosti prahov detekcie vývozov")
    logger.info("=" * 70)
...
def resample_time_series(df: pd.DataFrame) -> pd.DataFrame:
    ...
    doplnením s limitom dve vzorky (ffill(limit=2)). Metadáta
    (container_type, trash_type, district,
...
def filter_containers_by_min_samples(
    ...
    logger.info(
        f"Filter {tag}: ponechaných kontajnerov {info['kept_containers']:,}, "
        f"ponechaných riadkov {info['kept_rows']:,}, "
        f"odstránených kontajnerov {info['dropped_containers']:,}"
    )
...
def verify_no_data_leakage(df: pd.DataFrame, target_col: str, feature_cols: List[str]) -> bool:
    ...
    """
    logger.info("Kontrolujem únik informácií z budúcnosti...")
    ...
    for cid in sample_containers:
        ...
        for col in lag_cols[:5]:
            if not pd.isna(first_row[col]):
                logger.warning(f"Možný únik informácií: {col} nie je NaN v prvom riadku kontajnera {cid}")
                issues_found += 1

    if issues_found == 0:
        logger.info("Kontrola úniku informácií prešla")
        return True
    else:
        logger.warning(f"Kontrola úniku informácií našla problémové prípady: {issues_found}")
        return False
...
def check_feature_correlations(
    ...
    if high_corr:
        logger.warning(f"Nájdené vysoko korelované dvojice príznakov: {len(high_corr)} (>{threshold})")
        for col1, col2, corr in high_corr[:5]:
            logger.warning(f"  {col1} <-> {col2}: {corr:.3f}")
    else:
        logger.info(f"Nenašli sa vysoko korelované príznaky (>{threshold})")
