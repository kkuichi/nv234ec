...
def load_and_preprocess_data(filepath: str) -> pd.DataFrame:
    ...
    logger.info(f"Removed {initial - len(df):,} rows with unknown or missing trash type")

    # Odstránenie neplatných hodnôt plnosti.
    initial = len(df)
    df = df[(df['percent_calculated'] >= 0) & (df['percent_calculated'] <= 100)]
    logger.info(f"Removed {initial - len(df):,} rows with invalid percent")
    ...
    if 'temperature' in df.columns:
        ...
        if temp_outliers > 0:
            logger.info(f"Set {temp_outliers:,} temperature outliers to NaN")
    else:
    ...
    logger.info(f"Final: {len(df):,} rows, {df['container_id'].nunique()} containers")

    cap_dist = df.groupby('capacity_class')['container_id'].nunique()
    logger.info(f"Capacity distribution:\n{cap_dist.to_string()}")
