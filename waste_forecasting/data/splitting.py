...
def get_unified_test_containers(df: pd.DataFrame, seed: Optional[int] = None) -> set:
    ...
    vždy vráti rovnakú množinu, takže výsledky zostanú reprodukovateľné
    ...
    logger.info(f"Vybraných testovacích kontajnerov: {len(test_containers)} (seed={seed})")
...
def temporal_train_val_split_per_container(
    ...
    simuluje budúce predikcie - tréning prebieha na minulosti,
    validácia na neskorších pozorovaniach.
    """
...
def create_container_holdout_cv_folds(
    ...
    for fold_idx in range(n_folds):
        ...
        if "measured_at_utc" in train_df.columns and "measured_at_utc" in val_df.columns:
            ...
            logger.info(f"Aplikovaný časový rez CV. cutoff={cutoff}, train_rows={len(train_df)}, val_rows={len(val_df)}")

        folds.append((train_df, val_df))
        logger.info(f"  Fold {fold_idx+1}: tréning {len(train_df):,}, validácia {len(val_df):,}")
...
def create_expanding_window_cv_folds(
    ...
    for fold_idx in range(n_folds):
        ...
        if len(train_df) > 0 and len(val_df) > 0:
            folds.append((train_df, val_df))
            logger.info(f"  Fold {fold_idx+1} (rastúce okno): tréning {len(train_df):,}, validácia {len(val_df):,}")
